...
#include <private/bootNetLoad.h>   /* mxl added */
...
#define SIO_CHAR(x)  \
        *(volatile unsigned char *)0xbfe001e0 = (x); BIG_LOOP    /* added by moxiaolu for debug */
...
/*******************************************************************************
*
* bootAppMemModify - modify memory
*
...
LOCAL int bootAppMemModify32bits
    (
    char * cmd
    )
    {
    char * adrs;
    char line [160 + 1];   /* leave room for EOS */
    ...
    if (getArg (&cmd, (int *) &adrs, HEX, !OPT) == OK)
        {
        /* round down to word boundary */
    
        for (adrs = (char *) ((int) adrs & 0xfffffffe); /* start on even addr */
             ;                      /* FOREVER */
            adrs = (char *) (((long *) adrs) + 1))    /* bump as short ptr */
            {
            /* prompt for substitution */
    
            bootAppPrintf ("%06x:  %08x-", 
                          (int) adrs, 
                          (*(long *)adrs) ,
                          0,0,0,0);
            ...
            if (fioRdString (STD_IN, line, 160) == EOF)
                break;
        
            line [160] = EOS;  /* make sure input line has EOS */
            ...
            * (long *) adrs = value;       /* assign new value */
            }
...
extern bootGoPrototype * bootGoOverrideFunc;
extern UINT32 bootMemBase;
/* MMU disable call */
#ifndef _WRS_BOOT_DISABLE_MMU
#define _WRS_BOOT_DISABLE_MMU  VM_ENABLE(FALSE)
#endif

extern BOOL bootDisableMmuBeforeEntry;
...
LOCAL STATUS bootFtpLoad_mxl
    ...
    if (bootLoadModule (fd, pEntry) != OK)
		{
    	bootAppPrintf("Load Module failed\n",0,0,0,0,0,0);
        goto readErr;
		}
...
LOCAL STATUS netLoad_mxl
    ...
    if (sysFlags & SYSFLG_TFTP)     /* use tftp to get image */
        ...
        {
			{
			return (bootFtpLoad_mxl(hostAdr, fileName, usr, passwd, pEntry));
			}
        }
    else
        {
...
LOCAL STATUS bootNetLoad_mxl
    ...
    if (bootDhcpcAdrAdjustFcn != NULL)
        ...
        if (sysFlags & SYSFLG_AUTOCONFIG)
            ...
            bootDhcpcEnableFcn (sysBootParams.unitNum, sysBootParams.bootDev);
    ...
    if (netLoad_mxl (sysBootParams.had, sysBootParams.bootFile, sysBootParams.usr,
        ...
        if (bootDhcpcDisableFcn != NULL)
            bootDhcpcDisableFcn(sysBootParams.unitNum, sysBootParams.bootDev);
        status = OK;
        #endif
        }
...
STATUS bootLoad_mxl
    ...
    */
    ...
    /* while ((currentLoader != NULL) &&
        ...
        bootNetLoad_mxl(bootString, pEntry);
...
int bootAppBootFileLoadAndGo_mxl
    (
    char * cmd
    )
    {
...
LOCAL int bootAppParamsChange_mxl
	(
	char * cmd
	)
	{
    ...
    if (strlen(sysBootLine) <= BOOT_CMD_MAX_LINE)
        ...
        /* if (bootNetSettingsUpdateFunc != NULL)*/
            {
            /* networking enabled, setup network settings */
            (void) bootNetSettingsUpdate_mxl(&oldBootParams, &sysBootParams); 
            }
        }
    else
...
STATUS bootNetSettingsUpdate_mxl (BOOT_PARAMS * pOldBootParams,  BOOT_PARAMS * pBootParams   )
{
    ...
    bsp_printf("bootNetSettingsUpdate_mxl : 1 \r\n");
    ...
    bsp_printf("bootNetSettingsUpdate_mxl : 2 \r\n");
    ...
    if ((strcmp (oldBootDev, "scsi") != 0) &&
        ...
        (strcmp (oldBootDev, "usb") != 0)  &&
        (strcmp (oldBootDev, "tsfs") != 0)  &&
        (strcmp (oldBootDev, "tffs") != 0) )
        {

        printf("bootNetSettingsUpdate_mxl : unit: %d, %s \r\n" , 
                    pOldBootParams->unitNum,  pOldBootParams->bootDev);
                    
        /* cleanup old settings */
        if (ipDetach(pOldBootParams->unitNum, pOldBootParams->bootDev) == ERROR)
            {
    ...
    bsp_printf("bootNetSettingsUpdate_mxl : 3 \r\n");
    ...
    if ((strcmp (bootDev, "scsi") != 0) &&
        ...
        if ((strcmp (bootDev, "bp") == 0) ||
            (strcmp (bootDev, "sm") == 0))
            {
            strcpy (ipAddr, pBootParams->bad); /* backplane IP parameter */
            }
        else
            {
            strcpy (ipAddr, pBootParams->ead); /* ethernet IP parameter */
            }

    bsp_printf("bootNetSettingsUpdate_mxl : 4 \r\n");
        ...
        if (netmaskResult <= 0)
            ...
            char    maskStr[16];    /* netmask ascii string */

            /* Extract the 8 MSB of IP addr, or digits before the period */
            i = strspn(ipAddr, "0123456789");
            strncpy(&msbStr[0], ipAddr, i);    /* get MSB of IP addr */
            ...
            */
            if ((0 <= msbVal) && (msbVal < 128))
                ...
                bootAppPrintf ("Error reading target netmask information.\n"
                               ...
                               0,0,0,0,0,0);
                netmask = 0xFF000000;       /* Class A network default mask */
                }
                ...
                bootAppPrintf ("Error reading target netmask information.\n"
                               ...
                               0,0,0,0,0,0);
                netmask = 0xFFFF0000;       /* Class B network default mask */
                }
            else
                {
                bootAppPrintf ("Error reading target netmask information.\n"
                               ...
                               0,0,0,0,0,0);
                netmask = 0xFFFFFF00;       /* Class C network default mask */
                }

            /* Add default mask string to IP addr */
            (void)sprintf(maskStr, ":%8x", netmask);
            (void)strcat(ipAddr, maskStr);
            bootAppPrintf("Using default network mask = 0x%X\n", netmask,
                          0,0,0,0,0);
            }
            bsp_printf("bootNetSettingsUpdate_mxl : 5 \r\n");

        printf("bootNetSettingsUpdate_mxl  new: unit: %d, %s \r\n" , 
            pBootParams->unitNum,  pBootParams->bootDev);                    

        /* enable new device */
        if (ipAttach(pBootParams->unitNum, pBootParams->bootDev) == ERROR)
            {
            bootAppPrintf("Error attaching to network device\n",
                          0,0,0,0,0,0);
            bsp_printf("bootNetSettingsUpdate_mxl : 6 \r\n");
            return (ERROR);
            }

            bsp_printf("bootNetSettingsUpdate_mxl : 7 \r\n");

        printf("bootNetSettingsUpdate_mxl ipAddr  : %s , %s \r\n" , 
            ipAddr,  pBootParams->gad);    
        /* configure new network settings */
        if (usrNetBootConfig (pBootParams->bootDev, 
                              pBootParams->unitNum, 
                              ipAddr, 
                              netmask, 
            ...
            bootAppPrintf("Error configuring network device\n",
                          0,0,0,0,0,0);
            bsp_printf("bootNetSettingsUpdate_mxl : 8 \r\n");
            return (ERROR);
            }
        }
            bsp_printf("bootNetSettingsUpdate_mxl : 9 \r\n");
    return (OK);
}
...
# undef INCLUDE_SNS_MP_RTP
    ...
    bootCommandHandlerAdd("r", bootAppMemModify32bits, BOOT_CMD_MATCH_STRICT,
         "r adrs", "- modify memory 32bits");
    bootCommandHandlerAdd("b", bootAppGo_mxl, BOOT_CMD_MATCH_STRICT,
         "b adrs", "- go to  memory directly ");

    bootCommandHandlerAdd("k", bootAppParamsChange_mxl, BOOT_CMD_MATCH_STRICT,
 		"k",  "- change boot params");


#if 1
     bootCommandHandlerAdd("j", bootAppBootFileLoadAndGo_mxl, BOOT_CMD_MATCH_LOOSE,
              "j ", "- = @  fire  (load and go)  ");
#endif
