...
VXB_DMA_TAG_ID vxbDmaBufTagParentGet
    ...
    {
#ifdef UNSUPPORTED_FUTURE_ENHANCEMENT
...
VXB_DMA_TAG_ID vxbDmaBufTagCreate
    ...
    if ( parent == NULL )
        {
        /* look for parent in VxBus parent */

        /* find regbase, if any */

        for ( i = 0 ; i < MAX_REGBASE ; i++ )
            if ( (bus_addr_t)pInst->pRegBase[i] == lowAddr )
        break;

        /* if regbase found, get associated parent tag */

        if ( i < MAX_REGBASE )
            parent = vxbDmaBufTagParentGet(pInst, i);
        else
            parent = NULL;
        }
    else
...
STATUS vxbDmaBufMapLoad
    ...
    for (i = 0; bufLen > 0; i++)
        ...
        map->fragList[i].frag = (void *)((int)ADDR_TRANS_FROM_VIRT ((void *)pCur) | 0x80000000); /* CFG, 20110920, add 0x80000000 */
...
STATUS vxbDmaBufMapMblkLoad
	...
	if (len == 0)
	    ...
	    map->fragList[nSegs].frag = (void *)((int)ADDR_TRANS_FROM_VIRT((void *)pCur) | 0x80000000); /* CFG, 20110920, add 0x80000000 */
